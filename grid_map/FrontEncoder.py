...
class Front_fea_Encoder(nn.Module):
    ...
    def __init__(self, num_layers, img_ht, img_wt, pretrained=True):
        super(Front_fea_Encoder, self).__init__()

        if_front_encoder = True
        self.resnet_encoder = ResnetEncoder(num_layers, pretrained, if_front_encoder)
    ...
    def forward(self, x):
        ...
        if verbose: print("#Encoder# out after resnet_encoder: ", x.shape)  #([B, 128, 32, 64])  only use layer_1 and layer_2 
        return x 
...
if __name__ == "__main__":
    num_minibatch = 2
    input_img = torch.randn(num_minibatch, 3, 256, 512).cuda(0)
    encoder_net = Front_fea_Encoder(18, 256, 512, True).cuda(0)
    out = encoder_net(input_img)  
    print(out.shape)